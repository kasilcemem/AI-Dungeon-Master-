...
def battle(player, enemy: dict) -> tuple:
    player_atk = random.randint(8, 20) + player.level * 3
    enemy_atk = enemy["atk"] + random.randint(0, 5)
    enemy["hp"] -= player_atk
    won = enemy["hp"] <= 0
    ...
    if won:
        ...
        result = (
            ...
            f"💔 {enemy['name']} {enemy_atk} hasar verdi! HP: {player.hp}"
        )
    return result, won
